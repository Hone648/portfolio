...
This repository is at Slice 9E: Home Security case-study narrative. The Home Security story now leads with the selective-alert problem and follows one event along the camera-to-notification path instead of reading as a service inventory. The narrative explains service boundaries, detection treated as an automation input rather than an alert, contextual area and time filtering, MQTT integration at the producer, broker, and consumer level, boundary-by-boundary troubleshooting, local-operation tradeoffs, and residential privacy as a design constraint. Registry facts, project status, the no-repository boundary, and the recorded limitations remain unchanged, and the shared layout still renders the registry limitations after the narrative. The project remains an operational private residential integration rather than a commercial security product or professionally monitored alarm. No Home Security visuals were added. Slice 9F, visual personality and presentation, is the next planned slice. Slice 8C, Home Security visual evidence, is postponed rather than cancelled.

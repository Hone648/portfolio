...
| Slice 16F: Forkfolio architecture and workflow diagrams | Unblocked by completed Slice 16E but deliberately deferred during Slice 17 repositioning. Add high-signal diagrams that explain Forkfolio engineering depth without exposing private source code or private infrastructure. | Do not expose private paths, identifiers, hostnames, credentials, secret infrastructure, unsupported services, or production infrastructure that does not yet exist. |
| Slice 16G: Portfolio-wide Forkfolio integration | Deferred during Slice 17 repositioning. Integrate Forkfolio consistently across skills, homepage/project hierarchy, featured wording, README documentation, and resume/project references where the current architecture requires it. | Keep one source of truth for project facts; do not broaden skill claims, create a public source link, duplicate facts, or silently promote status. |
| Slice 16H: Validation and production release | Deferred during Slice 17 repositioning. Validate and release the completed case-study modernization and Forkfolio addition. | Automated tests are not accessibility-conformance claims; do not add unrelated frameworks, dependencies, or redesign work during release validation. |

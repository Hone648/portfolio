...
| Slice 16G: Portfolio-wide Forkfolio integration | Current. Integrate Forkfolio across project hierarchy, current project-backed skill relationships, homepage supplemental evidence, README documentation, and registry-driven Resume/About references while preserving the four-card homepage evidence balance and Forkfolio's active-development/private-source boundaries. | Keep one source of truth for project facts; do not broaden skill claims, create a public source link, duplicate facts, or silently promote status. |
| Slice 16H: Validation and production release | Pending behind 16G. Validate and release the completed case-study modernization and Forkfolio addition. | Automated tests are not accessibility-conformance claims; do not add unrelated frameworks, dependencies, or redesign work during release validation. |

...
| `/diagrams/forkfolio-tenant-authorization-boundary.svg` | Explains tenant management access, role context, tenant authorization, tenant-owned content domains, the Wagtail/custom-Django responsibility split, and PostgreSQL as the shared system of record. | Derived from the approved Forkfolio evidence contract and previously reviewed implementation evidence. | Private source paths, module layout, internal IDs, tenant/customer identifiers, hostnames, credentials, providers, deployment topology, production infrastructure, and any implication that conceptual boxes are independently deployed services. |
| `/diagrams/forkfolio-release-publication-flow.svg` | Explains editable tenant-owned state, preview/submission/review/approval, optional application-level scheduling, immutable release publication, public serving, and future edits proceeding toward a later release without mutating historical released state. | Derived from the approved Forkfolio evidence contract and previously reviewed implementation evidence. | Production scheduler infrastructure, deployment topology, backup/restore, infrastructure rollback, hosting/provider details, and production-operation claims. |
...
- The Forkfolio diagrams are code-authored explanatory SVGs derived from the approved evidence contract and previously reviewed implementation evidence.
- Forkfolio diagrams embed no private source excerpts, include no external resources or scripts, and represent application/workflow abstractions rather than production topology.

...
The public production URL is `https://portfolio-swart-rho-44.vercel.app`. HTTPS redirects correctly on the production origin. A Google Search Console HTML verification tag is configured through the root Next.js metadata object. The tag's scope is property ownership only; it does not establish indexing, rankings, traffic, analytics, or search performance. Google Analytics, Tag Manager, Vercel Analytics, and Speed Insights remain absent or disabled, and no custom domain is configured. Sitemap submission and Search Console verification remain post-deployment operator steps until actually completed. Deployment adds no database, backend service, authentication, CMS, admin dashboard, or runtime GitHub API call.
...
`content/project-metadata.ts` owns structured project facts, while `lib/projects.ts` provides synchronous, pure read helpers. Registry order controls default display order, and public status labels are centralized so presentation code does not duplicate them. The current default project order is newBudget, Forkfolio, Home Security and Automation Lab, and Unicos; `/projects` and Resume consume `getProjects()` and inherit that order.
...
The homepage builds `Selected engineering evidence` from explicit `getProjectBySlug()` and `getCareerEntryById()` lookups rather than from featured project cards. Its four primary evidence blocks remain newBudget, Home Security and Automation Lab, SPEA, and avionics systems experience from Advantage Aircraft and the U.S. Navy. Forkfolio is surfaced in that section as supplemental current software evidence, not as a fifth evidence card. Unicos is intentionally absent from the homepage primary evidence set, while `/projects` consumes `getProjects()` and remains the complete browsing surface for all published project case studies. Project metadata `featured` flags are now newBudget true, Forkfolio true, Home Security true, and Unicos false; `getFeaturedProjects()` remains available as a project helper even though it is not the homepage source.

`ProjectCard`, `ProjectGrid`, and `StatusBadge` are presentation-only Server Components. Cards receive complete project records, grids choose the appropriate heading level and density, and full and compact status labels remain centralised in the project registry. Project-aware `data-project` attributes drive decorative accents only. Index cards use a wider editorial layout that collapses in DOM order on smaller screens. Cards retain their registry-owned copy, status, links, and published case-study routes.
...
`lib/project-case-studies.ts` maps the published `newbudget`, `unicos`, `home-security-lab`, and `forkfolio` slugs to statically imported MDX components in project order and assigns each one an explicit presentation mode. All four current case studies use the `visual-walkthrough` presentation, while the `legacy` mode type remains available for future or archival content. The `/projects/[slug]` route uses that mapping for static parameters, metadata, content selection, presentation selection, and unknown-slug rejection; `dynamicParams = false` ensures only published slugs are generated. Project link records distinguish internal case studies from live and external destinations, and route availability must be updated with registry metadata. Home Security uses public repository metadata for `nvr-infrastructure`, so the case study exposes the reviewed source link while project cards may also present it as an external destination. Forkfolio uses private repository metadata with no public source URL and no live application link.

## Visual Evidence

`content/project-visuals.ts` owns visual paths, kinds, native dimensions, titles, alternative text, captions, and evidence notes. The registry remains partial: newBudget, Unicos, and Forkfolio have approved visual groups, Home Security remains unregistered, and projects without registered visuals return an empty readonly collection.

`ProjectEvidenceGallery` remains a presentation-only Server Component for the legacy presentation mode. Shared server-rendered visual group primitives own the figures, visible card captions, evidence notes, thumbnail images, and direct full-size links for both the legacy gallery and the visual walkthrough. `InlineProjectVisualGroup` selects registered visual groups by project slug and group ID so MDX can interleave approved visuals without duplicating paths, dimensions, titles, alt text, captions, or evidence notes; newBudget, Unicos, and Forkfolio use those registered groups inline. Home Security uses the same case-study presentation mode with no registered visual evidence. Forkfolio now has registered screenshot visual groups from authentic local-development captures using fictional demo data plus public-safe tenant/authorization and publication-flow diagrams. The Forkfolio diagrams are explanatory application/workflow abstractions that omit private source structure and unsupported production infrastructure. Thumbnail anchors become native buttons, while `EvidenceLightbox` is a narrowly scoped Client Component that owns selected state, native-dialog synchronisation, focus restoration, backdrop handling, and temporary scroll locking. The server passes only the serialisable visual fields required by the modal; evidence notes remain exclusively in the server-rendered cards.

The lightbox renders only the selected enlarged `next/image`, preserves intrinsic dimensions and the existing SVG and `development-admin` optimisation bypasses, and retains an `Open original asset` link. It includes no next/previous navigation, carousel, zoom, pan, swipe, URL state, or hidden full-size image set. newBudget screenshots remain under `public/images/newbudget`, Unicos screenshots remain under `public/images/unicos`, Forkfolio screenshots live under `public/images/forkfolio`, and code-authored newBudget, Unicos, and Forkfolio SVGs remain under `public/diagrams`. There is no runtime image API or external image dependency.

The newBudget screenshots are authentic owner-approved application captures using approved demonstration records. The newBudget month-first planning SVG is a code-authored explanatory diagram derived from reviewed implementation and narrative evidence; it is not a production-infrastructure map. The Unicos screenshots remain approved local-development captures, and the Unicos diagrams remain sanitized explanatory views. The Forkfolio screenshots are authentic local-development captures from a fictional demo showroom and demonstrate visible implemented interfaces without implying production deployment, production readiness, or live-customer operation. The Forkfolio tenant/authorization and publication-flow diagrams are explanatory application/workflow abstractions derived from approved public-safe evidence; they omit private source details, private infrastructure, and unsupported production infrastructure. The SVGs are explanatory representations derived from reviewed evidence and omit private source details and unsupported production infrastructure. Visuals supplement rather than replace structured evidence, narrative, status labels, or limitations.

## Profile Content and Presentation

`content/career-history.ts` owns the selected career entries and transferable-strength mappings. It keeps one consolidated SPEA entry that represents two separate periods of employment without publishing either period's dates. `content/site-content.ts` owns the career-transition narrative and other approved public profile copy, including identity, positioning, education direction, certification, and contact language. `content/skills.ts` remains reserved for typed, current project-backed skill groups and their related project slugs.

`/about`, `/resume`, and `/contact` are static Server Component routes. Shared profile components provide the route-owned page heading, detailed or compact current project-backed skill presentations, selected career entries, and transferable-strength mappings without introducing a generic page-builder or timeline abstraction. About and Resume both use `content/skills.ts` for current project-backed skill groups, while transferable strengths remain sourced from `content/career-history.ts`. Current project-backed skills and historical or transferable evidence remain distinct structured sources.

About presents transferable strengths linked to Resume experience anchors and then the current project-backed skill groups. About and Resume skill sections obtain Forkfolio links through `content/skills.ts` related project slugs and the shared `SkillGroups` renderer; Resume does not duplicate Forkfolio facts and receives selected-project order from the central project registry. Resume presents summary, engineering strengths, selected technical experience, selected projects, current project-backed skills, education and training, certification, and contact links. Contact is a static `mailto:` and GitHub surface; there is no form workflow, downloadable document, Client Component, or new runtime architecture.

...
The homepage builds `Selected engineering evidence` from explicit `getProjectBySlug()` and `getCareerEntryById()` lookups rather than from featured project cards. Its four examples are newBudget, Home Security and Automation Lab, SPEA, and avionics systems experience from Advantage Aircraft and the U.S. Navy. Unicos is intentionally absent from the homepage evidence set, while `/projects` consumes `getProjects()` and remains the complete browsing surface for all published project case studies. Project metadata `featured` flags remain intact, and `getFeaturedProjects()` remains available as a project helper even though it is not the homepage source.
...
About presents transferable strengths linked to Resume experience anchors and then the current project-backed skill groups. Resume presents summary, engineering strengths, selected technical experience, selected projects, current project-backed skills, education and training, certification, and contact links. Contact is a static `mailto:` and GitHub surface; there is no form workflow, downloadable document, Client Component, or new runtime architecture.

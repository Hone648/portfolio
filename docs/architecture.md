...
`lib/project-case-studies.ts` maps the published `newbudget`, `unicos`, and `home-security-lab` slugs to statically imported MDX components in project order and assigns each one an explicit presentation mode. `newbudget` uses the `visual-walkthrough` presentation as the Slice 16A reference implementation; Unicos and Home Security remain on the `legacy` presentation until a later approved migration slice. The `/projects/[slug]` route uses that mapping for static parameters, metadata, content selection, presentation selection, and unknown-slug rejection; `dynamicParams = false` ensures only published slugs are generated. Project link records distinguish internal case studies from live and external destinations, and route availability must be updated with registry metadata. Home Security uses public repository metadata for `nvr-infrastructure`, so the case study exposes the reviewed source link while project cards may also present it as an external destination.
